/*! ut_live_chat. MIT License. WebTorrent LLC <https://webtorrent.io/opensource> */
...
module.exports = () => {
  class utLiveChat extends EventEmitter {
    constructor (wire) {
      super()
    ...
    onExtendedHandshake (handshake) {
      if (!handshake.m || !handshake.m.ut_live_chat) {
        return this.emit('warning', new Error('Peer does not support ut_live_chat'))
      }
    }
    ...
    onMessage (buf) {
      ...
      try {
        ...
        this.emit('chat' , message.toString())
      } catch (err) {
    ...
    _sendMessage () {
      ...
      // send PEX message
      this._wire.extended('ut_live_chat', {
        added: added,
        'added.f': addedFlags,
        dropped: dropped,
  ...
  utLiveChat.prototype.name = 'ut_live_chat'
